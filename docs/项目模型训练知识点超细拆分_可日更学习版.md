...
### 0.1 全链路代码导航索引（先收藏，学习时按图索骥）

> 学习顺序建议：先看「训练脚本入口」-> 再看「dataset 产出张量」-> 再看「trainer_utils 公共能力」。

| 学习模块 | 训练入口脚本 | 关键数据类/函数 | 关键公共函数 |
|---|---|---|---|
| Tokenizer | `trainer/train_tokenizer.py` | `train_tokenizer` / `eval_tokenizer` | - |
| Pretrain | `trainer/train_pretrain.py` | `PretrainDataset.__getitem__` | `get_lr` / `lm_checkpoint` / `init_model` / `SkipBatchSampler` |
| Full SFT | `trainer/train_full_sft.py` | `SFTDataset.create_chat_prompt` / `SFTDataset.generate_labels` | 同上 |
| LoRA SFT | `trainer/train_lora.py` | `SFTDataset` | `get_lr` / `lm_checkpoint` + `apply_lora` / `save_lora` |
| DPO | `trainer/train_dpo.py` | `DPODataset.__getitem__` | `logits_to_log_probs` / `dpo_loss` |
| Reason | `trainer/train_reason.py` | `SFTDataset`（含标签掩码思路） | 训练循环同 pretrain/sft，额外有思维标签权重逻辑 |
| Distillation | `trainer/train_distillation.py` | `PretrainDataset` | `distillation_loss` |
| PPO | `trainer/train_ppo.py` | `RLAIFDataset.__getitem__` | `compute_gae` / `ppo_train_epoch` |
| GRPO | `trainer/train_grpo.py` | `RLAIFDataset.__getitem__` | `calculate_rewards` / `grpo_train_epoch` |
| SPO | `trainer/train_spo.py` | `RLAIFDataset.__getitem__` | `AutoAdaptiveValueTracker` / `spo_train_epoch` |
| Eval/推理 | `eval_llm.py` | `tokenizer.apply_chat_template` | `init_model`（eval 版本） |
...
**代码定位（先看这 3 处）**：
- `dataset/lm_dataset.py` -> `class PretrainDataset`
- `PretrainDataset.__getitem__`（文本编码、补齐、标签构造）
- `trainer/train_pretrain.py`（如何消费 `input_ids/labels`）
...
**代码定位（先看这 4 处）**：
- `dataset/lm_dataset.py` -> `class SFTDataset`
- `SFTDataset.create_chat_prompt`（调用 `tokenizer.apply_chat_template`）
- `SFTDataset.generate_labels`（assistant 监督区间）
- `trainer/train_full_sft.py` / `trainer/train_lora.py`（训练入口）
...
**代码定位（先看这 3 处）**：
- `dataset/lm_dataset.py` -> `class DPODataset`
- `DPODataset.__getitem__`（chosen/rejected 双路编码）
- `trainer/train_dpo.py`（对比损失计算）
...
**代码定位（先看这 3 处）**：
- `dataset/lm_dataset.py` -> `class RLAIFDataset`
- `RLAIFDataset.create_chat_prompt`（prompt 构造）
- `trainer/train_ppo.py` / `trainer/train_grpo.py` / `trainer/train_spo.py`（生成回答+奖励）
...
**代码定位**：`trainer/trainer_utils.py::get_lr`，在 `train_pretrain.py` / `train_full_sft.py` / `train_lora.py` / `train_reason.py` / `train_dpo.py` 中被调用。
...
**代码定位**：`trainer/trainer_utils.py::lm_checkpoint`，各训练脚本都有“保存分支 + resume 加载分支”。
...
**代码定位**：`trainer/trainer_utils.py::init_model`，训练与评测都围绕它做模型与 tokenizer 初始化。
...
**代码定位**：`trainer/trainer_utils.py::SkipBatchSampler`，用于 resume 时跳过已训练 batch。
...
**代码定位（建议按顺序读）**：
1. `trainer/train_pretrain.py::train_epoch`（主循环）
2. `with autocast_ctx`（混合精度）
3. `loss = loss / args.accumulation_steps`（梯度累积）
4. `clip_grad_norm_` + `optimizer.step()` + `scaler.update()`（更新）
5. `lm_checkpoint(...)`（保存）
...
**代码定位**：
- `trainer/train_full_sft.py::train_epoch`
- `dataset/lm_dataset.py::SFTDataset.create_chat_prompt`
- `dataset/lm_dataset.py::SFTDataset.generate_labels`
...
**代码定位**：
- `trainer/train_lora.py` 中 `apply_lora(model)`
- `lora_params = [p for ... if p.requires_grad]`
...
- `trainer/train_dpo.py::train_epoch`（policy/ref 前向与冻结逻辑）
...
**代码定位**：
- `trainer/train_reason.py::train_epoch`（loss mask 与加权位置）
- `dataset/lm_dataset.py::SFTDataset`（基础标签掩码来源）
...
**代码定位**：
- `trainer/train_distillation.py::distillation_loss`
- `trainer/train_distillation.py::train_epoch`（teacher/student 双前向）
...
**代码定位**：
- `trainer/train_ppo.py::compute_gae`
- `trainer/train_ppo.py::ppo_train_epoch`
- `dataset/lm_dataset.py::RLAIFDataset.__getitem__`
...
**代码定位**：
- `trainer/train_grpo.py::calculate_rewards`
- `trainer/train_grpo.py::grpo_train_epoch`
- `completion_mask`、`grouped_rewards`、`advantages` 相关代码块
...
**代码定位**：
- `trainer/train_spo.py::AutoAdaptiveValueTracker`
- `trainer/train_spo.py::spo_train_epoch`
- `rho_mode` 与 KL/优势估计相关代码块
...
**代码定位**：
- `trainer/train_tokenizer.py::train_tokenizer`
- `trainer/train_tokenizer.py::eval_tokenizer`
- `eval_llm.py` 中 `tokenizer.apply_chat_template` 的推理侧使用
...
## 12. 从 0 到能独立训练 MiniMind：人工智能“课本级”全知识底座（数学+技术+编程）

> 这一章是给“完全 0 基础”的你准备的。目标不是让你背术语，而是让你知道：
> 1) 每个知识点为什么存在；2) 学到什么程度算够用；3) 如何立刻映射到 MiniMind 项目。

### 12.1 总体地图：你到底要学哪些知识

你需要的知识可以分成 6 层（从下往上）：

1. **编程与工程基础**：Python、Linux、Git、环境管理、调试
2. **数学基础**：线性代数、概率统计、微积分、数值计算
3. **机器学习基础**：损失函数、优化、泛化、评估
4. **深度学习基础**：反向传播、网络结构、正则化、训练技巧
5. **LLM 专项**：Tokenizer、Transformer、Pretrain/SFT/对齐/RL
6. **LLM 工程化**：显存管理、实验设计、评测、部署、复盘

你现在的学习原则：

- **不追求一次懂透**，追求“今天能解释 + 能动手 + 能产出”。
- 每个知识点按 4 个层次学：
  - 白话直觉
  - 最小公式
  - 代码位置
...
**学到什么程度算够用**：
- 你能看懂训练脚本参数区并自己改参数。
- 你能读取一个 jsonl 并打印前 3 条样本。

**MiniMind 对应位置**：
- 参数解析：`argparse`（所有 `trainer/train_*.py`）
- 数据读取：`dataset/lm_dataset.py`

#### B. Linux 命令行必备

- 路径：`pwd`、`cd`
- 文件：`cat`、`sed`、`head`、`tail`
- 查找：`rg`
- 进程：`ps`、`nvidia-smi`

**学到什么程度算够用**：
- 你能独立进入目录、查看日志、定位报错文件。

#### C. Git 必备（你的学习资产管理器）

- `git status`、`git add`、`git commit`、`git log`
- 用 commit 记录“今天学了什么、改了什么、结论是什么”
...
### 12.3 数学知识底座（按“够用优先”，不是考研优先）

> 你不用一次学成数学家。你要的是：看到 loss、梯度、概率分布时不再害怕。
...
**为什么必须学**：
- Attention、Embedding、Linear 层都是矩阵运算。
...
**MiniMind 落地点**：
- 模型前向的 logits 输出本质是线性投影 + softmax 前分数。

**最小练习**：
1. 手算一个 `2x2` 矩阵乘法。
2. 解释“为什么维度不对就会报错”。

#### 12.3.2 概率统计（loss 与评估的语言）
...
**MiniMind 落地点**：
- Pretrain/SFT 本质是最大化正确 token 的概率（等价最小化交叉熵）。

**最小练习**：
1. 给 3 个候选词概率，算哪个词会被采样概率更高。
2. 用白话解释“为什么交叉熵小=模型更靠谱”。
...
#### 12.3.4 数值计算（训练稳定性的关键）

**你要掌握**：
- float32/float16/bfloat16 区别
- 上溢/下溢、NaN
- 归一化的意义

**MiniMind 落地点**：
- `autocast`、`GradScaler`、`dtype` 参数。

**最小练习**：
1. 记录一次 float16 训练是否更省显存。
2. 写出 NaN 排查清单（学习率、梯度、mask）。

---

### 12.4 机器学习基础（把“训练”变成可解释工程）
...
**你要会做**：
- 看曲线判断属于哪一种
- 给出一个调整动作（数据、模型、正则、学习率）

#### 12.4.3 常见损失函数

- MSE（回归）
- Cross Entropy（分类/语言模型）
- KL Divergence（蒸馏/分布对齐）
...
### 12.6 LLM 专项知识（课本难点，给你白话打通）

#### 12.6.1 Tokenizer 与词表

- 模型不懂“字义”，只懂 token id 与统计关系
- 词表大小影响 embedding 参数量与表达颗粒度

**MiniMind 落地点**：
- `trainer/train_tokenizer.py`
- `model/tokenizer.json`

#### 12.6.2 Transformer 核心
...
#### 12.6.8 知识蒸馏（Distillation）

- 学老师的概率分布（软标签），不是只背标准答案

#### 12.6.9 强化学习（PPO/GRPO/SPO）
...
### 12.8 课本知识 -> MiniMind 文件映射表（你按图索骥就不迷路）

- 数据处理：`dataset/lm_dataset.py`
- 通用训练工具：`trainer/trainer_utils.py`
- 预训练：`trainer/train_pretrain.py`
- 全参数SFT：`trainer/train_full_sft.py`
- LoRA：`trainer/train_lora.py`
- DPO：`trainer/train_dpo.py`
- 推理蒸馏：`trainer/train_reason.py`
- 白盒蒸馏：`trainer/train_distillation.py`
...
### 12.9 给 0 基础用户的“学习深度分级”标准（防止学过头）
...
#### Level 3（能分析）
- 能解释结果好坏原因，并给出下一步调参方案。

#### Level 4（能教学）
- 能把同样知识讲给另一个新手听懂。

你的目标不是直接冲 Level 4，而是每周把 2~3 个点从 L1 推到 L2。

---

### 12.10 120 天“课本知识补全计划”（每天 60~90 分钟）

#### 第 1-30 天：编程+数学打底
- Python 基础 + 张量 shape 直觉 + 导数直觉
- 目标：能看懂训练脚本

#### 第 31-60 天：机器学习+深度学习核心
- 损失函数、优化、反向传播、泛化
- 目标：能解释 loss 曲线变化

#### 第 61-90 天：LLM 主线
- Tokenizer、Transformer、Pretrain/SFT/DPO
- 目标：能跑通核心训练链路

#### 第 91-120 天：对齐与工程化
- Distillation、PPO/GRPO/SPO、评测与复盘
- 目标：能做一个可展示的小项目

---

### 12.11 每日学习记录模板（课本知识版）
...
### 12.12 最后给你的学习心法（非常重要）

1. **先用白话讲明白，再碰公式**。
2. **先跑通最小闭环，再追求大而全**。
3. **每次只改一个变量，才有结论价值**。
4. **每天有产出（笔记/图/视频）比“学很久没输出”更重要**。
5. **你不是在背 AI，你是在训练“可复现的解决问题能力”**。

> 如果你按这章执行，你会从“听不懂术语”逐步进入“看得懂代码、讲得出原理、做得出实验、交得出项目”的状态，这就是从 0 到 AI 实战者的真实路径。

---

## 13. MIT/哈佛课程强度与专家能力强化版（把“会学”升级为“会研究、会交付、会教学”）

> 你问得非常关键：目前内容还不等于 MIT/哈佛顶尖课程强度，更不等于“专家级”。
> 这一章给你一个更高标准：
> - **MIT/哈佛学生级**：能严谨复现、做对照实验、解释失败原因、写规范技术报告。
> - **专家级**：能提出新问题、设计新方法、完成多轮迭代并形成可迁移的方法论。

### 13.1 先对齐“名校级”与“专家级”的可验证标准

#### A. MIT/哈佛学生级（你至少要达到）

1. **理论表达能力**：能解释 Transformer、DPO、PPO 的核心假设与局限。
2. **复现能力**：能复现至少 3 条训练链路（如 SFT、DPO、GRPO）。
3. **实验设计能力**：每次实验有明确假设、控制变量、可复现实验记录。
4. **负结果解释能力**：能清晰说明失败来自数据、超参还是实现细节。
5. **技术写作能力**：能写出结构化 report（背景、方法、结果、误差分析、后续计划）。

#### B. 专家级（你后续冲刺）

1. **问题定义能力**：提出有价值的新问题，而非只调参。
2. **方法创新能力**：在 loss、采样、奖励、数据配比上提出改进并验证。
3. **跨系统能力**：训练、评测、部署、监控一体化。
4. **知识迁移能力**：把方法迁移到新任务、新数据、新场景。
5. **教学与领导能力**：能指导新人并形成标准化流程。

---

### 13.2 强化数学与理论深度（从“会用”到“会推导”）
...
- KL 散度、JS 散度、本质差异
- 偏差-方差权衡
- 置信区间与显著性检验（A/B 结果不靠感觉）

**进阶要求**：
- 能回答“模型 A 比 B 好，统计上是否显著”。
...
**进阶要求**：
- 能把“语言建模”解释成“压缩与预测”的统一问题。

---

### 13.3 强化算法理解深度（不仅会跑，还要会批判）

#### 13.3.1 Transformer 强化

- Attention 复杂度、长上下文瓶颈
- KV Cache 原理与推理加速
- RoPE 外推风险与长度泛化

#### 13.3.2 对齐算法强化

- SFT、DPO、PPO、GRPO、SPO 的目标函数差异
...
**专家向要求**：
- 你能说清“为什么这个方法在某任务有效，在另一个任务失效”。

---

### 13.4 强化工程能力（名校课程最容易忽视，但创业最关键）
...
构建“问题->证据->推断->修复->复验”闭环：
- 不是“猜哪里错了”，而是“基于证据定位问题”。

---

### 13.5 强化“研究方法论”（这是从学生到专家的分水岭）

#### 13.5.1 假设驱动（Hypothesis-driven）
...
### 13.6 进阶阅读与复现清单（向名校课程对齐）

#### A. 必读论文（按优先级）

1. Attention Is All You Need（Transformer）
2. LoRA（参数高效微调）
3. InstructGPT / RLHF 相关论文
4. DPO 原论文
5. PPO 原论文
6. 你当前关注方向的最新工作（每月 2 篇）
...
### 13.7 MIT/哈佛强度 24 周强化路线（在你现有计划上加压）

#### 第 1-8 周：理论强化 + 严格笔记
- 每周 1 个理论主题（线代/概率/优化/信息论）
- 每周 1 次“5 分钟白板讲解录屏”
- 目标：能脱稿讲清核心公式直觉

#### 第 9-16 周：算法复现 + 对照实验
- 每周 1 个训练脚本深挖（SFT/DPO/GRPO/PPO）
- 每周 2 组对照实验（含负结果）
- 目标：形成标准实验报告

#### 第 17-24 周：项目研究化 + 公开输出
- 选定一个垂直场景（教育/客服/医疗等）
- 建立评测基准 + 多轮迭代
- 每两周发布一次技术复盘长文
- 目标：达到“可答辩、可展示、可商业化”
...
### 13.8 专家级补强：你还需要新增的 8 项能力

1. **数据观**：知道什么数据决定了上限。
2. **指标观**：知道哪些指标在误导你。
3. **成本观**：知道 1% 效果提升是否值得 3 倍成本。
4. **系统观**：训练、推理、产品、运营要联动。
5. **安全观**：关注拒答、偏见、幻觉与合规。
6. **产品观**：模型价值必须映射真实用户需求。
7. **商业观**：内容、社群、课程、服务形成飞轮。
8. **组织观**：未来带团队时能标准化协作。

---

### 13.9 你的“毕业答辩标准”（达到就接近名校优秀项目）
...
### 13.10 现实校准：你现在在哪、下一步怎么走

如果你当前是“0 基础”：
- 你还没到 MIT/哈佛学生水平，也还没到专家水平，这是正常的。
- 你现在要做的不是焦虑，而是执行：
  1) 用第 11 章任务卡打基础行动力
  2) 用第 12 章补全课本知识底座
  3) 用本章第 13 章进入研究化与专家化训练

> 只要连续 6~12 个月高质量执行，你会从“不会”变成“能做”，再到“能讲、能带、能商业化交付”。这才是你真正要追求的竞争力。
